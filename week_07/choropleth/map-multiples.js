...
Promise.all([
  ...
  createChart(data, "2018", '#row1'); // for each year, run createChart for each year
  createChart(data, "2019", '#row1');
  createChart(data, "2020", '#row2');
  createChart(data, "2021", '#row2');
});

function createChart(allData, year, elemId) { // wrap data in function -- for each year, run createChart for each year
