...
Promise.all([
  ...
  const counties = topojson.feature(us, us.objects.counties); // using topojson lib
  ...
  svg.append("g")
    ...
    .join("path")
    .attr("fill", d => (d.id in dataById) ? color(dataById[d.id].rate) : '#ccc') // ? = if-else; if no data, fill with grey
    .attr("d", path)
